...
requiredUtils = ["git", "wget"]
for reqUtil in requiredUtils:
    shpath = shutil.which(reqUtil)
    print(shpath)
    if not shpath or len(shpath) == 0:
        print(f"{reqUtil} is not installed, install it and rerun the installation script.")
        if os.name != "posix":
            exit()
        if input(f"Do you want to install {reqUtil} ? [y/n] ") == "n":
            exit()
        if os.system(f"sudo apt install {reqUtil}") != 0:
            print(f"Failed to install {reqUtil}.")
            exit()    
        print(f"Successfully installed {reqUtil}.")
